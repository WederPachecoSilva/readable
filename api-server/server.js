  ...
  <pre>
    ...
    POST /categories/:id
      ...
      PARAMS:
        ...
        name - Category name
        path: the path that will be used for routing

    DELETE /category/:id
      USAGE:
        Sets the deleted flag for a category to 'true'.

      PARAMS: 
        id - UUID of the specific category
...
app.delete('/categories/:id', (req, res) => {
  categories.disable(req.token, req.params.id).then(
    data => res.send(data),
    error => {
      console.error(error);
      res.status(500).send({
